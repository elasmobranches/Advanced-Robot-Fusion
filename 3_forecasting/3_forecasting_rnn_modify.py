...
class RNNPredictor(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        # RNN 레이어 정의 (batch_first=True)
        self.rnn = nn.RNN(input_dim, hidden_dim, num_layers, batch_first=True)

        # 마지막 RNN 출력을 예측할 output_dim 차원으로 매핑하는 선형 레이어
        # RNN 출력은 (batch_size, seq_length, hidden_dim) 이므로,
    ...
    def forward(self, x):
        ...
        # hidden shape: (num_layers, batch_size, hidden_dim) - RNN은 cell state 없음
        out, hidden = self.rnn(x)

        # 시퀀스의 마지막 스텝의 hidden state를 가져와 fc 레이어에 입력
        # out[:, -1, :] shape: (batch_size, hidden_dim)
        out = self.fc(out[:, -1, :]) # 마지막 스텝 출력 사용
...
# 모델, 손실 함수, 옵티마이저 정의
rnn_model = RNNPredictor(input_dim, hidden_dim, output_dim, num_layers).to(device)
criterion = nn.MSELoss() # 예측 문제이므로 MSE 사용
optimizer = optim.Adam(rnn_model.parameters(), lr=1e-3) # Adam 옵티마이저

# 학습 파라미터
num_epochs = 250 # 에폭 수를 조정
# 조기 종료 (Early Stopping)를 위한 변수
best_val_loss = float('inf')
patience = 25 # 검증 손실 개선이 없을 때 기다릴 에폭 수
epochs_no_improve = 0


print(f"RNN 모델 학습 시작 ({num_epochs} 에폭)...")
# 학습 루프
for epoch in range(num_epochs):
    ...
    for batch_x, batch_y in Train_Loader:
        ...
        prediction = rnn_model(batch_x) # 모델 순전파
    ...
    # 검증 데이터로 모델 평가
    rnn_model.eval() # 평가 모드
    total_val_loss = 0
    with torch.no_grad(): # 그래디언트 계산 비활성화
        for batch_x_val, batch_y_val in Val_Loader:
            prediction_val = rnn_model(batch_x_val)
    ...
    if avg_val_loss < best_val_loss:
        ...
        epochs_no_improve = 0
        # 필요하다면 모델 가중치 저장: torch.save(rnn_model.state_dict(), 'best_rnn_predictor.pth')
    else:
        epochs_no_improve += 1
...
with torch.no_grad(): # 그래디언트 계산 비활성화
    for batch_x, batch_y in Test_Loader:
        ...
        output = rnn_model(batch_x) # 예측 수행
...
plt.title("Temperature Prediction (True vs. Predicted) - RNN")
