...
class GRUPredictor(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        # GRU 레이어 정의 (batch_first=True)
        self.gru = nn.GRU(input_dim, hidden_dim, num_layers, batch_first=True)

        # 마지막 GRU 출력을 예측할 output_dim 차원으로 매핑하는 선형 레이어
    ...
    def forward(self, x):
        ...
        # 시퀀스의 마지막 스텝 hidden state를 사용
        out = self.fc(out[:, -1, :])

        return out

# 모델, 손실 함수, 옵티마이저 정의
gru_model = GRUPredictor(input_dim, hidden_dim, output_dim, num_layers).to(device)
criterion = nn.MSELoss()  # 예측 문제이므로 MSE 사용
optimizer = optim.Adam(gru_model.parameters(), lr=1e-3)  # Adam 옵티마이저
...
print(f"GRU 모델 학습 시작 ({num_epochs} 에폭)...")
# 학습 루프
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for batch_x_val, batch_y_val in Val_Loader:
            prediction_val = gru_model(batch_x_val)
...
with torch.no_grad():
    for batch_x, batch_y in Test_Loader:
        ...
        batch_mse = torch.mean((output - batch_y)**2, dim=1)
...
plt.plot(ground_truth.ravel(), label="True temp_mean")
plt.plot(prediction.ravel(), label="Predicted temp_mean (GRU)")
plt.title("Temperature Prediction (True vs. Predicted) - GRU")
...
print("\n### 특정 시퀀스 예측 상세 시각화 (GRU 모델) ###")
num_sequences_to_plot = min(5, len(X_test))
...
for i in range(num_sequences_to_plot):
    ...
    plt.plot(time_steps, prediction[i], label="Predicted (GRU)")
    plt.title(f"Sequence {i+1} Prediction (GRU)")
